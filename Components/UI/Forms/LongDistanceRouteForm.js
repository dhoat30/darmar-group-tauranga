...
export default function LongDistanceRouteForm({
  ...
  hideTitle = false,
   routeId
    ...
    const { data } = useRouteCard(routeId);
  ...
  const submitHandler = (e) => {
    ...
    const dataPayload = {
      ...
      \nProperty Type: ${
       ...
       \nRoute Listed On Move Card: ${data?.movingFrom?.label || ""} to ${data?.movingTo?.label || ""}
       \nDate Listed On Move Card: ${movingCardDate || ""}
       \nTruck Size Listed On Move Card: ${data?.attributes?.truck_size || ""}
       \nNumber Of Movers Listed On Move Card: ${data?.attributes?.number_of_movers || ""}
       \nTruck Space Left: ${data?.spareCapacity || ""}
       \nStatus Listed On Move Card: ${data?.status || ""}
       \nDescription On Move Card: ${data?.description || ""}
       \n Message: ${
        formData.message
      } `,
    };
